...
CREATE OR REPLACE FUNCTION osm.ca_pm_abs_pm(IN highway numeric, IN ca_pm numeric, IN in_county varchar)
returns numeric
as
$BODY$
DECLARE
...
WITH
    cnty as (select st_transform(st_buffer(st_transform(the_geom,32611),1),4326)
             as the_geom
             from carb_counties_aligned_03 c
             where countyfp = $3),
...
';
  EXECUTE var_sql  INTO ret  USING highway,ca_pm,in_county;
  RETURN ret;
END;
$BODY$
  LANGUAGE 'plpgsql' IMMUTABLE STRICT;

ALTER FUNCTION osm.ca_pm_abs_pm(IN highway numeric, IN ca_pm numeric, IN in_county varchar)
OWNER TO postgres;
